...
class BibTeXEntry(BaseModel):
    ...
    entry_type: str = Field(
        description="Entry type (article, book, inproceedings, etc.)")
    ...
    booktitle: Optional[str] = Field(
        default=None, description="Book title (for proceedings)")
    ...
    extra_fields: Dict[str, str] = Field(
        default_factory=dict, description="Other BibTeX fields")
...
def validate_bibtex_entry(entry: BibTeXEntry) -> tuple[bool, List[str]]:
    ...
    required = required_fields.get(
        entry.entry_type.lower(), required_fields["misc"])

    for field in required:
        value = getattr(entry, field, None)
        if not value:
            errors.append(
                f"Missing required field for {entry.entry_type}: {field}")
...
def merge_bibtex_entries(
    ...
    for field, value in secondary.model_dump().items():
        if field in ["authors", "keywords"]:
            ...
            # Merge dicts
            merged_data[field] = {
                **secondary.extra_fields, **primary.extra_fields}
        else:
