...
class PDFSection(BaseModel):
    ...
    level: int = Field(
        default=1, description="Heading level (1=main, 2=sub, etc.)")
...
class PDFReference(BaseModel):
    ...
    title: Optional[str] = Field(
        default=None, description="Paper title if parsed")
    authors: Optional[List[str]] = Field(
        default=None, description="Author list if parsed")
    year: Optional[int] = Field(
        default=None, description="Publication year if parsed")
    ...
    try:
        ...
        if parsed_pdf is None:
            ...
            try:
                ...
                logger.warning(
                    f"[PDF Parser] pdfplumber failed: {e}, trying PyPDF2")
